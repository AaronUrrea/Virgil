...
module.exports.run = async(bot, message, args, player, channel) => {
    ...
    if(args[0] === '' || args[0] === ' ' || args[0] === 'help' || args[0] === undefined ){
        ...
        .setDescription((player.nickname == null ? player.user.username : player.nickname) + ', to use ?suggestion:')
        ...
        .setColor('#ff9900')
        .setTitle('111th Manticore Company')
        .setDescription((player.nickname == null ? player.user.username : player.nickname) + ', are you sure you want to suggest this?')
        .attachFiles(['./attachments/UNSC.png', './attachments/Manticore.png'])
        .setAuthor('UNSC', 'attachment://UNSC.png')
        .setThumbnail('attachment://Manticore.png')
        .addFields(
        ...
        .then(async collected => {
            ...
            if(reaction.emoji.name === '✔️'){
                let result = await bot.channels.cache.get("728505413682856038").send(new Discord.MessageEmbed()
                    .setColor('#ff9900')
                    .setTitle('111th Manticore Company')
                    .setDescription((player.nickname == null ? player.user.username : player.nickname) + ' suggests:')
                    .attachFiles(['./attachments/UNSC.png', './attachments/Manticore.png'])
                    .setAuthor('UNSC', 'attachment://UNSC.png')
                    .setThumbnail('attachment://Manticore.png') 
                    .addFields(
